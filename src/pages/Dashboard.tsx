import { Rocket, BookOpen, Clock, Award, Star, ArrowRight, Play, Code, Brain, ChevronRight, CheckCircle2 } from 'lucide-react'
...
export default function Dashboard() {
    ...
    const calculateModuleProgress = (moduleId: string) => {
        ...
        const completed = module.topics.reduce((acc, topic) => {
            return acc + topic.subtopics.filter(sub => completedSubtopics.includes(`${moduleId}-${sub.title}`)).length
        }, 0)
        return totalSubtopics === 0 ? 0 : Math.round((completed / totalSubtopics) * 100)
    }
        ...
        <div className="w-full px-6 md:px-12 space-y-10 pb-20">
            ...
            >
                ...
                <div className="relative z-10 max-w-xl">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black mb-4 tracking-tight">Master the Future of <span className="text-primary">Autonomous Agent Orchestration</span></h1>
                    ...
                    <div className="flex flex-wrap gap-4">
                        ...
                        <Link to="/syllabus">
                            <Button variant="outline" size="lg" className="border-slate-700 bg-transparent text-white hover:bg-slate-800 h-14 px-8 rounded-2xl">
                                View Syllabus
                            </Button>
                        </Link>
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="space-y-4">
                        {curriculum.slice(0, 3).map((module: Module, i: number) => {
                            ...
                            const isCurrent = module.id === lastModule
                            return (
                                <motion.div
                                    key={module.id}
                                ...
                                >
                                    <Link to={`/curriculum?module=${module.id}`}>
                                        <Card className={cn(
                                            ...
                                            isCurrent && "ring-2 ring-primary/20 bg-orange-50/10"
                                        )}>
                                            <CardContent className="p-5 flex items-center gap-5">
                                                <div className={cn(
                                                    "w-14 h-14 rounded-2xl flex items-center justify-center text-white shrink-0 transition-all",
                                                    progress === 100 ? "bg-emerald-500" : isCurrent ? "bg-primary" : "bg-slate-200 dark:bg-slate-800 text-slate-500"
                                                )}>
                                                    <span className="text-xl font-bold">
                                                        {progress === 100 ? <CheckCircle2 className="w-6 h-6" /> : module.number}
                                                    </span>
                                                </div>
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-1.5">
                                                        ...
                                                        {isCurrent && <span className="text-[10px] font-black text-primary uppercase tracking-widest bg-primary/10 px-2 py-0.5 rounded-full">Current</span>}
                                                    </div>
